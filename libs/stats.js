...
module.exports = function (logger, portalConfig, poolConfigs) {
    ...
    Object.keys(poolConfigs).forEach(function (coin) {
        ...
        for (var i = 0; i < redisClients.length; i++) {
            ...
            if (
                ...
                (client.client.path !== null &&
                    client.client.path === redisConfig.socket)
            ) {
    ...
    function gatherStatHistory() {
        ...
        // Updated method name and argument structure
        redisStats.zRangeByScore(
            ...
            (err, replies) => {
                ...
                _this.statHistory = replies
                    .map((reply) => JSON.parse(reply))
                    .sort((a, b) => a.time - b.time);
                _this.statHistory.forEach((stats) => addStatPoolHistory(stats));
            },
        );
    }
        ...
        async.each(
                                ...
                                var coinStats = {
                                    ...
                                    poolStats: {
                                        ...
                                        networkBlocks: replies[i + 2]
                                            ...
                                            : 0,
                                        networkSols: replies[i + 2]
                                            ? replies[i + 2].networkSols || 0
                                            : 0,
                                        networkSolsString:
            ...
            function (err) {
                ...
                Object.keys(allCoinStats).forEach(function (coin) {
                    ...
                    coinStats.hashrates.forEach(function (ins) {
                        ...
                        if (workerShares > 0) {
                            ...
                            if (worker in coinStats.workers) {
                                ...
                                if (
                                    lastShare >
                                    coinStats.workers[worker].lastShare
                                ) {
                                    coinStats.workers[worker].lastShare =
                                        lastShare;
                                }
                            ...
                            if (miner in coinStats.miners) {
                                ...
                                if (
                                    lastShare >
                                    coinStats.miners[miner].lastShare
                                ) {
                                    coinStats.miners[miner].lastShare =
                                        lastShare;
                                }
                    ...
                    var _blocktime = 160;
                    var _networkHashRate =
                        parseFloat(coinStats.poolStats.networkSols) * 1.2;
                    var _myHashRate = (coinStats.hashrate / 1000000) * 2;
                    coinStats.luckDays = (
                        ((_networkHashRate / _myHashRate) * _blocktime) /
                        (24 * 60 * 60)
                    ).toFixed(3);
                    coinStats.luckHours = (
                        ((_networkHashRate / _myHashRate) * _blocktime) /
                    ...
                    for (var worker in coinStats.workers) {
                        ...
                        coinStats.workers[worker].luckDays = (
                            ((_networkHashRate / _wHashRate) * _blocktime) /
                            (24 * 60 * 60)
                        ).toFixed(3);
                        ...
                        var miner = worker.split(".")[0];
                        if (miner in coinStats.miners) {
                            coinStats.workers[worker].currRoundTime =
                                coinStats.miners[miner].currRoundTime;
                        }
                    }
    ...
    function getReadableNetworkHashRateString(hashrate) {
        ...
        if (hashrate < 1000000) return "0 Sol";
        var byteUnits = [
            " Sol/s",
            " KSol/s",
            " MSol/s",

...
module.exports = function (logger) {
    ...
    var enabledPools = Object.keys(poolConfigs).filter(
        (coin) =>
        ...
        function (err, results) {
            if (err) {
                logger.error(
                    "PaymentProcessor",
                    "Error",
                    "An error occurred while setting up payment processing: " +
                        err,
                );
                return;
            }
            ...
            results.forEach(function (coin) {
                ...
                var redisConnectionDetails = poolOptions.redis.socket
...
function SetupForPool(logger, poolOptions, setupFinished) {
            ...
            function (result) {
                if (result.error) {
                    logger.error(
                        logSystem,
                        logComponent,
                        "Error with payment processing daemon (validateAddress) " +
            ...
            function (result) {
                if (result.error) {
                    logger.error(
                        logSystem,
                        logComponent,
                        "Error with payment processing daemon (validateTAddress) " +
                            JSON.stringify(result.error),
                    );
                    callback(true);
                } else if (!result.response || !result.response.ismine) {
            ...
            function (result) {
                if (result.error) {
                    logger.error(
                        logSystem,
                        logComponent,
                        "Error with payment processing daemon (validateZAddress) " +
                            JSON.stringify(result.error),
                    );
                    callback(true);
                } else if (!result.response || !result.response.ismine) {
    ...
    if (requireShielding === true) {
        ...
        async.parallel(
            [validateAddress, validateTAddress, getBalance],
            asyncComplete,
        );
    }
            ...
            daemon.cmd("getnetworkinfo", params, function (result) {
                if (
                    ...
                    logger.error(
                        logSystem,
                        logComponent,
                        "Error with RPC call getnetworkinfo " +
                            JSON.stringify(result[0].error),
                    );
                    return;
                }
                            ...
                            txDetails.forEach(function (tx, i) {
                                ...
                                if (tx.error && tx.error.code === -5) {
                                    ...
                                    if (badBlocks[round.txHash] >= 15) {
                                        logger.warning(
                                            logSystem,
                                            logComponent,
                                            "ERROR: Daemon reports invalid transaction: " +
                                                round.txHash,
                                        );
                                        ...
                                        logger.warning(
                                            logSystem,
                                            logComponent,
                                            `Abandoned block ${round.txHash} check ${badBlocks[round.txHash]}/15`,
                                        );
                                    }
                                    ...
                                    if (badBlocks[round.txHash] >= 15) {
                                        logger.warning(
                                            logSystem,
                                            logComponent,
                                            "ERROR: Daemon reports no details for transaction: " +
                                                round.txHash,
                                        );
                                        delete badBlocks[round.txHash];
                                        round.category = "kicked";
                                    ...
                                    return logger.error(
                                        logSystem,
                                        logComponent,
                                        `ERROR: Odd error with gettransaction ${round.txHash} ${JSON.stringify(tx)}`,
                                    );
                                }

                                // get the coin base generation tx
                                const generationTx = tx.result.details.filter(
                                    (tx) => tx.address === poolOptions.address,
                                )[0];
                                ...
                                if (!generationTx) {
                                    return logger.error(
                                        logSystem,
                                        logComponent,
                                        `ERROR: Missing output details to pool address for transaction ${round.txHash}`,
                                    );
                                }
        ...
        if (address.length >= 40) {
            logger.warning(
                ...
                "Invalid address " +
                    address +
                    ", convert to address " +
                    (poolOptions.invalidAddress || poolOptions.address),
            );

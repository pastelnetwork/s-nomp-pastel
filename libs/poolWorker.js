...
module.exports = function (logger) {
    ...
    Object.keys(poolConfigs).forEach(function (coin) {
        ...
        // We are skipping the legacy pw check logic because we are instead using the password to transmit the security token for the python mining validation api.
        var authorizeFN = function (ip, port, workerName, password, callback) {
            // Directly log the use of the password field for a new purpose (if necessary)
            logger.debug(
                logSystem,
                logComponent,
                logSubCat,
            ...
            callback({
                error: null,
                authorized: true, // Always authorized
                disconnect: false, // No need to disconnect
            });
        };
